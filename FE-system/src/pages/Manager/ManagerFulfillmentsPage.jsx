...
export function ManagerFulfillmentsPage() {
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-12 lg:items-end">
            <div className="space-y-2 lg:col-span-3">
              <label
                ...
                htmlFor="manager-fulfillment-status"
              >
                Trạng thái phiếu
              </label>
              <div className="relative">
                <select
                  id="manager-fulfillment-status"
            ...
            <div className="space-y-2 lg:col-span-3">
              <label
                className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"
                htmlFor="manager-fulfillment-order-id"
              >
                Mã đơn (orderId)
              </label>
              <input
                id="manager-fulfillment-order-id"
                type="text"
                inputMode="numeric"
                placeholder="VD. 1024"
                value={orderIdFilter}
            ...
            <div className="space-y-2 lg:col-span-3">
              <label
                className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"
                htmlFor="manager-fulfillment-worker-id"
              >
                Worker đã gán (ID)
              </label>
              <input
                id="manager-fulfillment-worker-id"
                type="text"
                inputMode="numeric"
                placeholder="VD. 7"
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {items.map((row, idx) => (
                  <tr
                    ...
                    onClick={() => navigate(`/manager/logistics/fulfillments/${row.id}`)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        navigate(`/manager/logistics/fulfillments/${row.id}`);
                      }
                    }}
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 pr-6 text-right align-middle">
                      <Button variant="outline" size="sm" className="h-8 gap-1 px-2.5" asChild>
                        <Link to={`/manager/sales/orders/${row.orderId}`} onClick={(e) => e.stopPropagation()}>
                          Xem đơn
