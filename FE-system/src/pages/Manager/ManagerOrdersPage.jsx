...
/**
 * Manager — Đơn hàng (`/api/admin/orders`) theo `manager/don-hang.md`.
 * Manager điều phối: gán Sales, hủy, cập nhật trạng thái. Chi tiết: `/manager/sales/orders/:id`.
 */
export function ManagerOrdersPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, paymentStatus, orderStatus, unassignedOnly, pageSize]);
  ...
  }, [accessToken, isAuthenticated, page, pageSize, debouncedSearch, paymentStatus, orderStatus]);
  ...
  const items = unassignedOnly ? allItems.filter((r) => r?.salesId == null) : allItems;
  ...
  const hasActiveFilters =
    Boolean(searchInput.trim()) || Boolean(paymentStatus) || Boolean(orderStatus) || unassignedOnly;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Gán Sales, cập nhật trạng thái, hủy đơn khi hợp lệ. API dùng chung với Admin Orders.
        </p>
      </div>
        ...
        <CardContent className="space-y-5 p-5 sm:p-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-12 lg:items-end">
            <div className="space-y-2 sm:col-span-2 lg:col-span-5">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400" htmlFor="mgr-order-search">
                Tìm kiếm
              </label>
            ...
            <div className="space-y-2 lg:col-span-3">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400" htmlFor="mgr-pay-status">
                Thanh toán
              </label>
              <div className="relative">
                <select id="mgr-pay-status" value={paymentStatus} onChange={(e) => setPaymentStatus(e.target.value)} className={fieldSelect}>
            ...
            <div className="space-y-2 lg:col-span-4">
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400" htmlFor="mgr-order-status">
                Trạng thái đơn
              </label>
              <div className="relative">
                <select id="mgr-order-status" value={orderStatus} onChange={(e) => setOrderStatus(e.target.value)} className={fieldSelect}>
          ...
          <div className="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <label className="ml-1 inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                <input
                  type="checkbox"
          ...
          <div>
            ...
            <CardDescription className="mt-1 text-xs sm:text-sm">
              {loading
                ...
                : `${totalCount.toLocaleString("vi-VN")} đơn trong hệ thống${
                    unassignedOnly ? ` · hiển thị ${items.length} chưa có Sales (trong trang)` : ""
                  }`}
            </CardDescription>
          </div>
            ...
            <table className="w-full min-w-[1040px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/95 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-900/80 dark:text-slate-400">
                  ...
                  <th className="px-5 py-3.5 text-right last:pr-6">Tổng</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {items.map((row, idx) => {
                  ...
                  const detailPath = orderId != null ? `${MANAGER_ORDERS_BASE}/${orderId}` : null;
                  return (
                    <tr
                      ...
                      <td className="px-5 py-3.5 align-middle text-xs">
                        {row.salesId != null ? (
                          <span className="font-medium text-slate-800 dark:text-slate-200">
                            {row.salesName ?? `#${row.salesId}`}
                          </span>
                        ) : (
                          <span className="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 font-semibold text-amber-800 ring-1 ring-amber-200/80 dark:bg-amber-950/40 dark:text-amber-200 dark:ring-amber-800/50">
                            Chưa gán
                          </span>
                        )}
                      </td>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {totalCount > 0 ? (
                <>
                  ...
                  trong tổng{" "}
                  <span className="font-medium text-slate-700 dark:text-slate-200">{totalCount.toLocaleString("vi-VN")}</span> đơn
                </>
              ) : (
