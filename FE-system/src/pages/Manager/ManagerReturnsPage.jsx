...
const RETURNS_BASE = "/manager/after-sales/returns";
...
/**
 * Danh sách đổi trả (Manager) — API admin returns. `defaultStatusLocked` (vd. `Pending`) cho màn «chờ duyệt».
 * @param {{ defaultStatusLocked?: string }} [props]
 */
export function ManagerReturnsPage({ defaultStatusLocked } = {}) {
  ...
  useEffect(() => {
    setPage(1);
  }, [statusForApi, type, customerId, orderId, fromDate, toDate, search, pageSize]);
  ...
  }, [accessToken, isAuthenticated, page, pageSize, statusForApi, type, customerId, orderId, fromDate, toDate, search]);
  ...
  const hasActiveFilters = Boolean(
    (!lockedStatus && status) || type || customerId.trim() || orderId.trim() || fromDate || toDate || searchInput.trim()
  );
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
            Manager duyệt / từ chối; bước hoàn tất nhận hàng do kho (StockManager) thực hiện — theo policy đổi trả.
          </p>
          {lockedStatus ? (
            <p className="mt-2 text-sm">
              <span className="rounded-md bg-amber-50 px-2 py-1 font-medium text-amber-900 ring-1 ring-amber-200/80 dark:bg-amber-950/30 dark:text-amber-100 dark:ring-amber-800/50">
                Đang lọc trạng thái: {lockedStatus}
              </span>
              <Link className="ml-3 text-sm font-medium text-violet-700 underline-offset-2 hover:underline dark:text-violet-400" to={RETURNS_BASE}>
                Xem tất cả phiếu
              </Link>
            </p>
          ) : null}
