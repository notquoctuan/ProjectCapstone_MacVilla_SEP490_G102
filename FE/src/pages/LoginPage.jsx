...
export function LoginPage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (emailNorm === 'customer@gmail.com') {
      login({ email: emailNorm, name: 'Customer' })
      navigate('/', { replace: true })
      return
    }
    if (emailNorm === 'company@gmail.com') {
      login({ email: emailNorm, name: 'Cty Việt Hàn' })
      navigate('/', { replace: true })
      return
    }
    // TODO: integrate real auth for other emails
  }
    ...
    <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 overflow-x-hidden">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-background-dark/80 backdrop-blur-md px-6 md:px-20 py-4 sticky top-0 z-50">
        <Link to="/" className="flex items-center gap-3">
          <div className="bg-primary p-1.5 rounded-lg text-white">
            <Icon name="kitchen" className="block text-2xl" />
          </div>
          ...
          <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center">
            <div className="mb-10 text-center md:text-left">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-base">
                Đăng nhập để quản lý đơn hàng và khám phá bộ sưu tập thiết bị
                cao cấp nhất.
              </p>
            </div>
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 rounded-lg shadow-lg shadow-primary/20 transition-all transform active:scale-[0.98] mt-4"
              >
                ĐĂNG NHẬP
              </button>
            </form>
      ...
      <footer className="px-6 md:px-20 py-8 text-center text-slate-500 dark:text-slate-400 text-sm">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <p>© 2024 HDG Việt Hàn. Tất cả các quyền được bảo lưu.</p>
