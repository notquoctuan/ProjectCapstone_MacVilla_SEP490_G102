...
const FULFILLMENTS_LIST_PATH = "/stock-manager/fulfillments";
const ORDERS_PATH = "/stock-manager/orders";
const INVENTORY_TX_PATH = "/stock-manager/inventory/transactions";
 ...
 */
export function StockManagerFulfillmentDetailPage() {
      ...
      {!loading && !error && detail ? (
        <>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {orderId != null && String(status).toLowerCase() === "shipped" ? (
              <Link
                to={INVENTORY_TX_PATH}
                className="inline-flex items-center justify-center rounded-lg border border-teal-200 bg-teal-50/80 px-4 py-2 text-sm font-medium text-teal-900 shadow-sm hover:bg-teal-100/80 dark:border-teal-800 dark:bg-teal-950/40 dark:text-teal-100 dark:hover:bg-teal-950/60"
              >
                Giao dịch kho (đối chiếu đơn #{String(orderId)})
              </Link>
            ) : null}
          </div>
        </>
