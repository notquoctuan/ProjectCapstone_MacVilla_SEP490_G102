...
const FULFILLMENTS_HUB = "/stock-manager/fulfillments";
...
/**
 * Stock Manager — danh sách phiếu xuất (`fulfillment.md`).
 * @param {{ initialAssignedWorkerId?: string }} [props] — khi có, cố định bộ lọc Worker (màn «theo worker»).
 */
export function StockManagerFulfillmentsPage({ initialAssignedWorkerId } = {}) {
  const navigate = useNavigate();
  const { accessToken, isAuthenticated, user } = useAuth();
  const lockedWorkerId =
    initialAssignedWorkerId != null && String(initialAssignedWorkerId).trim() !== ""
      ? String(initialAssignedWorkerId).trim()
      : null;
  ...
  const [workerIdFilter, setWorkerIdFilter] = useState(() => lockedWorkerId ?? "");
  ...
  useEffect(() => {
    if (lockedWorkerId) setWorkerIdFilter(lockedWorkerId);
  }, [lockedWorkerId]);
  ...
  const clearFilters = () => {
    ...
    setStatus("");
    setOrderIdFilter("");
    setWorkerIdFilter(lockedWorkerId ?? "");
  };
  const hasActiveFilters = Boolean(
    listMode === "queue" ||
      status ||
      orderIdFilter.trim() ||
      (!lockedWorkerId && workerIdFilter.trim())
  );
                ...
                {lockedWorkerId ? (
                  <p className="mt-2 text-xs sm:text-sm">
                    <span className="rounded-md bg-amber-50 px-2 py-1 font-medium text-amber-900 ring-1 ring-amber-200/80 dark:bg-amber-950/30 dark:text-amber-100 dark:ring-amber-800/50">
                      Đang lọc Worker ID: {lockedWorkerId}
                    </span>
                    <Link
                      className="ml-2 font-medium text-teal-700 underline-offset-2 hover:underline dark:text-teal-400"
                      to={FULFILLMENTS_HUB}
                    >
                      Xem tất cả phiếu
                    </Link>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-12 lg:items-end">
            <div className="space-y-2 lg:col-span-3">
              <label
                ...
                htmlFor="sm-fulfillment-status"
              >
                Trạng thái phiếu
              </label>
              <div className="relative">
                <select
                  id="sm-fulfillment-status"
            ...
            <div className="space-y-2 lg:col-span-3">
              <label
                className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"
                htmlFor="sm-fulfillment-order-id"
              >
                Mã đơn (orderId)
              </label>
              <input
                id="sm-fulfillment-order-id"
                type="text"
                inputMode="numeric"
                placeholder="VD. 1024"
                value={orderIdFilter}
            ...
            <div className="space-y-2 lg:col-span-3">
              <label
                className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"
                htmlFor="sm-fulfillment-worker-id"
              >
                Worker đã gán (ID)
              </label>
              <input
                id="sm-fulfillment-worker-id"
                type="text"
                inputMode="numeric"
                placeholder="VD. 7"
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {items.map((row, idx) => (
                  <tr
                    ...
                    onClick={() => navigate(`${FULFILLMENTS_HUB}/${row.id}`)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        navigate(`${FULFILLMENTS_HUB}/${row.id}`);
                      }
                    }}
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 pr-6 text-right align-middle">
                      <Button variant="outline" size="sm" className="h-8 gap-1 px-2.5" asChild>
                        <Link to={`/stock-manager/orders/${row.orderId}`} onClick={(e) => e.stopPropagation()}>
                          Xem đơn
