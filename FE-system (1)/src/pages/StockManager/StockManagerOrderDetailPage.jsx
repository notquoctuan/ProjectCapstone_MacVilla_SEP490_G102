...
import { ChevronRight, ExternalLink, Loader2, MapPin, Package, Plus, RotateCcw, Truck, UserRound } from "lucide-react";
...
const ORDERS_LIST_PATH = "/stock-manager/orders";
...
/** Đơn có thể tạo phiếu xuất — `stockmanager/fulfillment.md` (Confirmed) + giai đoạn vận hành kho. */
function orderAllowsFulfillmentCreation(order) {
  const s = order?.orderStatus ?? order?.status;
  return s === "Confirmed" || s === "Processing" || s === "ReadyToShip";
}
...
/**
 * Stock Manager — chi tiết đơn — GET /api/admin/orders/:id (`stockmanager/fulfillment.md`).
 */
export function StockManagerOrderDetailPage() {
  const { id } = useParams();
  ...
  const submitCreateFulfillment = async () => {
    ...
    try {
      ...
      if (newId != null && Number.isFinite(Number(newId))) {
        navigate(`/stock-manager/fulfillments/${Number(newId)}`);
      }
    } catch (e) {
      const msg =
        e instanceof ApiRequestError
                ...
                <CardHeader className="border-b border-slate-100 bg-teal-50/50 dark:border-slate-800 dark:bg-teal-950/20">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap items-center gap-2">
                      ...
                      <Link
                        to={`/stock-manager/returns?orderId=${encodeURIComponent(String(order.id))}${
                          ...
                          {fulfillments.map((row, idx) => (
                            ...
                            >
                              <td className="whitespace-nowrap px-5 py-3 pl-6 font-mono text-xs tabular-nums text-slate-500 dark:text-slate-400">
                                <Link
                                  to={`/stock-manager/fulfillments/${row.id}`}
